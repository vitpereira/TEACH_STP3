...
# Observe o across aqui, que me permite fazer o rowMeans para várias colunas 
  mutate(apoio_aprend = rowMeans(across(c(a1_apoio_aprend_1:a1_apoio_aprend_4))  , na.rm=TRUE), 
         ...
         cultura_sala_aula = rowMeans(across(c(a1_apoio_aprend_1:a1_expect_posit_3)) , na.rm=TRUE),
         instrucao         = rowMeans(across(c(a2_licao_clara_1:a2_racioc_crit_3)) , na.rm=TRUE),
         socio_emoc        = rowMeans(across(c(a3_autonomia_1:a3_cap_soc_colab_3)), na.rm=TRUE) )

saveRDS(teach_limpo4, file=paste0(tmp,"teach_construtos.RData" ))
